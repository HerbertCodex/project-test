...
function storeWith(overrides: Partial<RenewStore> = {}): {
  store: RenewStore;
  saved: Loan[];
} {
  const saved: Loan[] = [];
...
describe('Prolonger un pret', () => {
  const loanWith = (
    fields: { renewals?: number; lostAt?: Date },
    onSave: (loan: Loan) => void = () => undefined,
  ): RenewStore =>
    storeWith({
      save: (loan: Loan) => {
        onSave(loan);
  ...
  const renew = (store: RenewStore): RenewUseCase =>
    new RenewUseCase(store, DEFAULT_POLICY);
  ...
  it('repousse l echeance depuis AUJOURD HUI, pas depuis l ancienne echeance', async () => {
    ...
    expect(renewed.dueAt.getTime()).toBeGreaterThan(
      DUE.getTime() + 23 * 86_400_000,
    );
  });
  ...
  it('REFUS : un autre adherent a reserve le titre', async () => {
    const { store } = storeWith({
      waitingHolds: () =>
        Promise.resolve([
          new Hold({ titleId: 't1', memberId: 'm2', placedAt: OUT }),
        ]),
    });
    await expect(renew(store).execute(request)).rejects.toThrow(
      TitleIsHeldByAnother,
    );
  });
  ...
  it('REFUS : le plafond de prolongations est atteint', async () => {
    const store = loanWith({ renewals: DEFAULT_POLICY.renewalLimit });
    await expect(renew(store).execute(request)).rejects.toThrow(
      RenewalLimitReached,
    );
  });
  ...
  it('REFUS : le pret a bascule en perdu', async () => {
    const store = loanWith({ lostAt: LATE });
    await expect(renew(store).execute(request)).rejects.toThrow(
      LoanCannotBeRenewed,
    );
  });
  ...
  it('REFUS : l exemplaire n est pas en pret', async () => {
    ...
    await expect(renew(store).execute(request)).rejects.toThrow(NothingToRenew);
  });
  ...
  it('le compteur avance et ne peut pas etre remis a zero par une prolongation', async () => {
    const saved: Loan[] = [];
    const store = loanWith({ renewals: 2 }, (loan) => saved.push(loan));
