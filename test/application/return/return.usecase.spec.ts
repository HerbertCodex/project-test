...
const request: ReturnRequest = { copyId: 'c1', now: new Date('2026-03-20T10:00:00Z') };

/**
 * Un magasin en mémoire qui garde ce qu'on lui écrit.
 */
function storeWith(overrides: Partial<CirculationStore> = {}): {
  ...
  const store = {
    copyById: () => Promise.resolve(null),
    memberById: () => Promise.resolve(null),
...
describe('Rendre un exemplaire', () => {
  it('ferme le pret et rend l exemplaire empruntable, par la disponibilite DERIVEE', async () => {
    ...
    const outcome = await new ReturnUseCase(store, DEFAULT_POLICY).execute(request);
  ...
  it('REFUS : l exemplaire n est pas en pret', async () => {
    const { store } = storeWith({ openLoanOfCopy: () => Promise.resolve(null) });
    await expect(new ReturnUseCase(store, DEFAULT_POLICY).execute(request)).rejects.toThrow(
      CopyNotOnLoan,
    );
  });
  ...
  it('constate une dette egale aux jours de retard fois le bareme', async () => {
    ...
    const outcome = await new ReturnUseCase(store, DEFAULT_POLICY).execute(late);
  ...
  it('n expose aucun moyen de paiement', () => {
    const offenders = ['src/application/return']
      ...
      .filter((path) => /pay|charge|refund|PaymentGateway|stripe/i.test(readFileSync(path, 'utf8')));
