...
function storeWith(overrides: Partial<HoldStore> = {}): { store: HoldStore; saved: Hold[] } {
  const saved: Hold[] = [];
  const store = {
...
describe('Reserver un titre', () => {
  const place = (store: HoldStore): PlaceHoldUseCase => new PlaceHoldUseCase(store, DEFAULT_POLICY);
  ...
  it('REFUS 1 : l adherent detient deja un exemplaire du titre', async () => {
    const { store } = storeWith({ memberHoldsCopyOf: () => Promise.resolve(true) });
    await expect(place(store).execute(request)).rejects.toThrow(AlreadyHoldsACopy);
  });
  ...
  it('REFUS 2 : le plafond de reservations est atteint', async () => {
    ...
    await expect(place(store).execute(request)).rejects.toThrow(HoldCeilingReached);
  });
  ...
  it('REFUS 3 : les impayes depassent le seuil', async () => {
    ...
    await expect(place(store).execute(request)).rejects.toThrow(BlockedByDebtForHold);
  });

  it('REFUS 4 : le titre est disponible, il n y a rien a reserver', async () => {
    const { store } = storeWith({ availableCopiesOf: () => Promise.resolve(2) });
    await expect(place(store).execute(request)).rejects.toThrow(NothingToReserve);
  });
});
