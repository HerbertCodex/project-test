...
- `migrate(db)` applique chaque migration non appliquée dans sa propre transaction et fixe `user_version`. Il **refuse une base dont `user_version` dépasse `LATEST_SCHEMA_VERSION`** (erreur « Schéma de base en version … plus récent … »). Deux migrations existent à `f638f1e` : `LATEST_SCHEMA_VERSION` vaut 2.
...
- L'historique des prêts rendus est conservé sans limite à `f638f1e` : la conservation d'un an et la purge sont prévues à l'incrément 3 et ne sont pas implémentées.
